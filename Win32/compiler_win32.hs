{-
Adrian Duraj, 18.09.2008, win32 version
...
argCheck xs 
   | (head xs == "-o") && (head (tail (tail xs)) == "-keep-asm-file") && (length xs == 4) = do
      ...
      system ("nasm -f win32 " ++ makeShortName (last xs) ++ ".asm")
      system ("gcc -o " ++ (head (tail xs)) ++ " " ++ makeShortName (last xs) ++ ".obj")
      system ("del " ++ makeShortName (last xs) ++ ".obj")
   ...
   | (head xs == "-o") && (length xs == 3) = do
      ...
      system ("gcc -o " ++ (head (tail xs)) ++ " " ++ makeShortName (last xs) ++ ".obj")
      system ("del " ++ makeShortName (last xs) ++ ".obj")
      system ("del " ++ makeShortName (last xs) ++ ".asm")
      putStrLn "Compilation successful!"
      exitWith ExitSuccess
   | (head xs == "-keep-asm-file") && (length xs == 2)  = do
      ...
      case Main.parse prog of
         ...
         Right ast -> writeFile (makeShortName (last xs) ++ ".asm") $ generate ast
      system ("nasm -f win32 " ++ makeShortName (last xs) ++ ".asm")
      system ("gcc -o a.exe " ++ makeShortName (last xs) ++ ".obj")
      system ("del " ++ makeShortName (last xs) ++ ".obj")
      putStrLn "Compilation successful!"
      exitWith ExitSuccess
   | length xs == 1 = do 
